...
# list all decks from the database
  def index
    ...
    @percimen = Deck.where(:year_id => @user.student_year)
    #@sorted = Deck.order(params[:sort] + ' ' + params[:direction])
...
# Process and upload contents of the uploaded deck into the database
  ...
  private
    ...
    def upload_params
      params.require( :deck).permit( :author, :topic_id, :attachment, :class_name_id, :year_id, :professor_id, :deck_id, :notes)
    end
  ...
  def sort_column
    ...
    Deck.column_names.include?(params[:sort_column]) ? params[:sort_column] : "LOWER(topic_id)"
  end
