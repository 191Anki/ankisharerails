class CardsController < ApplicationController
  ...
  # Shows selected cards when "View" button is clicked on homepage
  def edit
    @card = Card.find(params[:id])
  end

  # Redirect so that when viewing cards, user can download the viewed deck
  def download_in_cards
    redirect_to  decks_repackage_path(:id =>@deck.id), notice: "Finished."    
  end
  ...
  # Updating cards
  # Also increments user's edit count
  def update
